...
# from app.api.dependencies import get_current_user  # We'll need this
...
async def create_tournament(
    ...
    try:
        tournament = await TournamentService.create_tournament(
            db, tournament_data, dummy_user_id  # current_user.id
        )
        return tournament
...
async def get_my_tournaments(
    ...
    tournaments = await TournamentService.get_tournaments_by_creator(
        db, dummy_user_id, skip, limit  # current_user.id
    )
    return tournaments
...
async def get_my_participations(
    ...
    participations = await TournamentParticipantService.get_user_participations(
        db, dummy_user_id, include_club_participations=True, skip=skip, limit=limit
    )
...
async def register_for_tournament(
    ...
    try:
        participant = await TournamentParticipantService.register_participant(
            db, tournament_id, participant_data, dummy_user_id
        )
        return participant
    except ValueError as e:
        raise HTTPException(
